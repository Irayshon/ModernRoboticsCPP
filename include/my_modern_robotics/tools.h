...
class Tools {
 public:
   ...
   * @param value double scalar value to test.
   * @return bool true if the magnitude is below the internal tolerance.
   */
  static bool NearZero(double value);

  /**
   * @brief Normalize a vector or matrix by its Frobenius norm.
   * @param V Eigen::MatrixXd input matrix or vector.
   * @return Eigen::MatrixXd normalized matrix/vector.
   */
  static Eigen::MatrixXd Normalize(Eigen::MatrixXd V);

  /**
   * @brief Invert a rotation matrix (transpose).
   * @param rot_matrix Eigen::MatrixXd rotation matrix.
   * @return Eigen::MatrixXd inverse rotation matrix.
   */
  static Eigen::MatrixXd RotInv(const Eigen::MatrixXd& rot_matrix);

  /**
   * @brief Convert a 3-vector to a 3x3 so(3) matrix.
   * @param omg Eigen::Vector3d angular velocity vector.
   * @return Eigen::Matrix3d so(3) matrix.
   */
  static Eigen::Matrix3d VecToso3(const Eigen::Vector3d& omg);

  /**
   * @brief Convert a 3x3 so(3) matrix to a 3-vector.
   * @param so3mat Eigen::MatrixXd so(3) matrix.
   * @return Eigen::Vector3d angular velocity vector.
   */
   ...
   * @param expc3 Eigen::Vector3d exponential coordinates.
   * @return Eigen::Vector4d [axis(3), theta].
   */
  static Eigen::Vector4d AxisAng3(const Eigen::Vector3d& expc3);

  /**
   * @brief Compute the matrix exponential of an so(3) matrix.
   * @param so3mat Eigen::Matrix3d so(3) matrix.
   * @return Eigen::Matrix3d rotation matrix in SO(3).
   */
   ...
   * @param R Eigen::Matrix3d rotation matrix in SO(3).
   * @return Eigen::Matrix3d so(3) matrix.
   */
   ...
   * @param R Eigen::Matrix3d rotation matrix.
   * @param p Eigen::Vector3d position vector.
   * @return Eigen::MatrixXd 4x4 homogeneous transform.
   */
   ...
   * @param T Eigen::MatrixXd 4x4 homogeneous transform.
   * @return std::vector<Eigen::MatrixXd> rotation (3x3) and position (3x1).
   */
  static std::vector<Eigen::MatrixXd> TransToRp(const Eigen::MatrixXd& T);

  /**
   * @brief Invert a homogeneous transform.
   * @param transform Eigen::MatrixXd 4x4 homogeneous transform.
   * @return Eigen::MatrixXd inverse transform.
   */
  static Eigen::MatrixXd TransInv(const Eigen::MatrixXd& transform);

  /**
   * @brief Convert a 6-vector to an se(3) matrix.
   * @param V Eigen::VectorXd 6-vector twist.
   * @return Eigen::MatrixXd 4x4 se(3) matrix.
   */
  static Eigen::MatrixXd VecTose3(const Eigen::VectorXd& V);

  /**
   * @brief Convert an se(3) matrix to a 6-vector.
   * @param T Eigen::MatrixXd 4x4 se(3) matrix.
   * @return Eigen::VectorXd 6-vector twist.
   */
   ...
   * @param T Eigen::MatrixXd 4x4 homogeneous transform.
   * @return Eigen::MatrixXd 6x6 adjoint matrix.
   */
  static Eigen::MatrixXd Adjoint(const Eigen::MatrixXd& T);

  /**
   * @brief Construct a screw axis from a point, direction, and pitch.
   * @param q Eigen::Vector3d point on the screw axis.
   * @param s Eigen::Vector3d unit direction of the screw axis.
   * @param h double screw pitch.
   * @return Eigen::VectorXd 6-vector screw axis.
   */
   ...
   * @param expc6 Eigen::VectorXd exponential coordinates (6-vector).
   * @return Eigen::VectorXd 7-vector [axis(6), theta].
   */
  static Eigen::VectorXd AxisAng6(const Eigen::VectorXd& expc6);

  /**
   * @brief Compute the matrix exponential of an se(3) matrix.
   * @param se3mat Eigen::MatrixXd 4x4 se(3) matrix.
   * @return Eigen::MatrixXd 4x4 homogeneous transform in SE(3).
   */
   ...
   * @param R Eigen::Matrix3d input matrix.
   * @return Eigen::Matrix3d rotation matrix in SO(3).
   */
   ...
   * @param T Eigen::MatrixXd input 4x4 matrix.
   * @return Eigen::MatrixXd homogeneous transform in SE(3).
   */
   ...
   * @param R Eigen::Matrix3d input matrix.
   * @return bool true if R is in SO(3).
   */
   ...
   * @param T Eigen::MatrixXd input matrix.
   * @return bool true if T is in SE(3).
   */
  static bool TestIfSE3(const Eigen::MatrixXd& T);

  /**
   * @brief Compute the space Jacobian.
   * @param Slist Eigen::MatrixXd 6xn matrix of screw axes in space frame.
   * @param thetalist Eigen::VectorXd n-vector of joint angles.
   * @return Eigen::MatrixXd 6xn Jacobian in space frame.
   */
   ...
   * @param Blist Eigen::MatrixXd 6xn matrix of screw axes in body frame.
   * @param thetalist Eigen::VectorXd n-vector of joint angles.
   * @return Eigen::MatrixXd 6xn Jacobian in body frame.
   */
   ...
   * @param V Eigen::VectorXd 6-vector twist.
   * @return Eigen::MatrixXd 6x6 matrix ad(V).
   */
  static Eigen::MatrixXd ad(const Eigen::VectorXd& V);

  /**
   * @brief Check if a Jacobian is near-singular.
   * @param J Eigen::MatrixXd Jacobian matrix.
   * @return bool true if the condition suggests singularity.
   */
   ...
   * @param J Eigen::MatrixXd Jacobian matrix.
   * @return double condition number (ratio of max/min singular value).
   */
